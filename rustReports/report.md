...
本项目严格遵循实验要求，**明确排除**以下不符合条件的项目类型，并通过实际开发内容证明项目的复杂度和原创性。

### 2.1 排除项对照说明

| 排除类型 | 本项目情况 | 证明依据 |
|---------|-----------|---------|
| **纯前端页面项目** | ❌ 不符合 | 本项目包含完整的 Rust 后端服务（5,046 行代码），前后端分离架构 |
| **简单 CRUD 管理系统** | ❌ 不符合 | 实现了编码生成、预留机制、序号回收等复杂业务逻辑 |
| **仅调用第三方 API 的包装器** | ❌ 不符合 | 自建数据库层、存储抽象层，非简单 API 转发 |
| **主要逻辑由 AI 生成的项目** | ❌ 不符合 | 核心业务逻辑经人工设计，代码结构体现工程思考 |
| **Rust 代码占比极低的项目** | ❌ 不符合 | Rust 后端代码 5,046 行，占项目总代码量 70%+ |

### 2.2 项目复杂度量化指标

| 指标 | 数值 | 说明 |
|------|------|------|
| **Rust 源码行数** | 5,046 行 | 纯后端代码，不含测试 |
| **测试代码行数** | 942 行 | 集成测试 + 单元测试 |
| **API 路由数量** | 60+ 个 | 传统 flat POST + RESTful 双风格 |
| **数据库表数量** | 8 张 | 含复杂关联和状态流转 |
| **外部依赖数量** | 20 个 | Axum、SQLx、S3 SDK 等核心库 |

### 2.3 复杂业务逻辑说明

#### 2.3.1 五段式编码系统

商品编码采用 `主分区-子分区-序号-数量-状态` 五段式设计，非简单自增 ID：

```
示例编码：A-B-0001-0015-C
├── A      主分区 (A-Z)
├── B      子分区 (A-Z)
├── 0001   序号 (1-9999，支持回收复用)
├── 0015   数量 (实时更新)
└── C      状态编码 (可自定义)
```
...
使用 `ImageStorage` trait 实现存储层解耦，便于扩展和测试：
...
使用 Tokio + Axum 全异步架构，SQLx 异步数据库操作，体现现代 Rust 异步生态。

### 2.5 与简单项目的核心区别

1. **架构复杂度**：非单文件应用，采用模块化分层架构
2. **业务逻辑**：包含状态机（预留机制）、算法（序号回收）、数据一致性保证
3. **技术深度**：使用 Rust 核心特性（所有权、trait、泛型、异步）
4. **工程规范**：完整的错误处理、中间件、测试覆盖
5. **自主设计**：核心业务逻辑经人工设计，非 AI 生成模板代码
